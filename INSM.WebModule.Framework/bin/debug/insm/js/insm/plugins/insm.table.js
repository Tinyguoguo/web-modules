...
*
* Called by $('identifier').insmTable(settings);
*
* File dependencies:
* jQuery 1.8.2
* jQuery UI 1.8.23
...
(function ($) {
    var methods = {
        init: function (options) {
            var $this = $(this);
            var _plugin = $this.data('insmTable');
            ...
            if (!_plugin) {
                _plugin = {
                    ...
                    callback: {
                        onSearch: function () {
                            $this.insmTable('filter');

                            $this.insmTable('sort');
                            $this.insmTable('populateTable');
                        },
                        onSort: function () {
                            $this.insmTable('sort');
                            $this.insmTable('populateTable');
            ...
            // Create sticky item
            _plugin.data.sticky = {};
            $this.insmTable('updateSticky');

            // Update data
            $this.insmTable('update', {
                items: _plugin.settings.items
            }).insmTable('filter').insmTable('populateTable');
            // Update pagination
            if (_plugin.settings.pagination) {
        ...
        remove: function (options) {
            ...
            if (_plugin.settings.items[options.id]) {
                ...
                $this.insmTable('updateSearchIndex');
                $this.insmTable('updateSticky');
                $this.insmTable('filter');
                $this.insmTable('populateTable');
            }
        ...
        update: function (options) {
            ...
            $.each(options.items, function (id, item) {
                ...
                if (typeof _plugin.settings.items[id]._tr !== 'undefined') {
                    var row = $this.insmTable('createRow', {
                        id: id,
                        item: item
                    });
            ...
            for (var i = _plugin.data.offset; i < lastItemIndex; i++) {
                if (_plugin.settings.items[_plugin.data.tableData[i]]) {
                    if (!_plugin.settings.items[_plugin.settings.items[_plugin.data.tableData[i]].id]._tr) {
                        ...
                        _plugin.settings.items[_plugin.settings.items[_plugin.data.tableData[i]].id]._tr = $this.insmTable('createRow', {
                            id: _plugin.settings.items[_plugin.data.tableData[i]].id,
                            item: _plugin.settings.items[_plugin.data.tableData[i]]
                        });
                    }
                    _plugin.htmlElements.tbody.append(_plugin.settings.items[_plugin.settings.items[_plugin.data.tableData[i]].id]._tr);
                }
            }
        ...
        destroy: function (options) {
            var $this = $(this);
            $this.data('insmTable', null).empty();

            return $this;
        }
    };

    $.fn.insmTable = function (method) {
        if (methods[method]) {
            return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            return methods.init.apply(this, arguments);
        } else {
            $.error('Method ' + method + ' does not exist on $.insmTable');
        }
    };

    $.insmTable = function (method) {
        return $('<div />').insmTable(arguments);
    };
})(jQuery);