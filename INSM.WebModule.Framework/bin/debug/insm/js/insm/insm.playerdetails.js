		...
		init: function (options) {
			...
			$.insmFramework('player', {
				...
				success: function (player) {
					...
					_plugin.htmlElements.playerTransfer.append(
						$('<table />').addClass('vertical no-border').append(
							...
							$('<tr />').append(
								...
								$('<td />').append(
									$('<div />').insmStatusBox({
										title: player.Transfer.OnDemandStatus ? player.Transfer.OnDemandStatus.State + (player.Transfer.OnDemandStatus.Message && player.Transfer.OnDemandStatus.Message.trim().length > 0 ? ': ' + player.Transfer.OnDemandStatus.Message : '') : 'Unknown',
						...
						$.each(addedMessageSamples, function (index, value) {
							...
							infoContent.append( 
								...
								$('<tr />').append(
									...
									$.each(player.Status.MessageLog.Log, function (key,stateMessage) {
					...
					/// PLAYER INFORMATION ////
					var fulfilmentStateDescription = $('<div />').addClass('insm-player-details-popup').append(
					...
					_plugin.htmlElements.playerInformation.html(
						$('<table />').addClass('vertical no-border').append(
							...
							$('<tr />').append(
								...
								$('<td />').append(
									$('<div />').insmStatusBox({
										...
										//tooltip: 'This is an aggregated state designed to show with reasonable confidence if the player has downloaded scheduled content and is healthy regardless of its current online state.\r\n The Fulfilment State looks at the player’s Sync State, On Demand Download State and Online State and intelligently taking a grace period into account decides if the player is OK or Not OK. The Fulfillment State will first move into a Warning Not OK and then an Error Not OK until it recovers to OK. The short Warning and long Error grace period are set with AMS Settings fulfilmentWarningLimitSeconds and fulfilmentErrorLimitSeconds. Specifically:If the Sync State is OK and On Demand Download State and Online State is or has been OK at some point looking back within the grace period then the Fulfilment state is OK.If the On Demand Download State is transferring for files that were all scheduled within the grace period then the Fulfilment state is OK. If the On Demand Download State is transferring for a file scheduled longer ago than the grace period then the Fulfilment state is Not OK If the Sync State was OK sometime within the grace period and no files are transferring or scheduled to be transferred then the Fulfilment state is OK. If changes in the CMS have been made some time ago which was longer than the grace period Sync State is not up to date then the Fulfilment state is Not OK. If the Sync State is OK (no change has been made in the CMS) and no file is transferring or scheduled to be transferred then the Fulfilment state is OK. If Online State is OK and Current State is error with any of the following errors then the Fulfilment State is Not OK: 1002 Error Screenshot black inside opening hours. 1011 Error No player engine or template scheduled 1013 Error No content. 1039 Error No playlist scheduled. 1041 Error No available assets. 1040 Error Unconfigured. 1042 Error No files in playlist',
							...
							$('<tr />').append(
								...
								$.each(player.Plugins, function (key, plugin) {
									statusList["plugin " + plugin.Name] = plugin;
								});
								$.each(player.DisplayDevices, function (key, displayDevice) {
									statusList["Virtual Display " + key] = displayDevice;
								});
								$.each(player.PlayerViews, function (key, playerView) {
									statusList["Channel " + key] = playerView;
								});
								$.each(player.Monitors, function (key, monitor) {
									statusList["Physical Display " + key] = monitor;
								});
